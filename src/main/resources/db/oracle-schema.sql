...
CREATE TABLE notice (
  ...
  CONSTRAINT fk_notice_author FOREIGN KEY (author_id) REFERENCES app_user (id)
);
...
CREATE TABLE qna_post (
  ...
  CONSTRAINT fk_qna_writer FOREIGN KEY (writer_id) REFERENCES app_user (id),
...
CREATE TABLE review (
  ...
  CONSTRAINT fk_review_writer FOREIGN KEY (writer_id) REFERENCES app_user (id),
  CONSTRAINT fk_review_product FOREIGN KEY (procedure_product_id) REFERENCES procedure_product (id),
...
CREATE TABLE review_attachment (
  ...
  CONSTRAINT fk_review_attachment_review FOREIGN KEY (review_id) REFERENCES review (id)
);
...
CREATE TABLE coupon (
  ...
  name VARCHAR2(80 CHAR) NOT NULL,
  discount_amount NUMBER(12, 0) NOT NULL,
  min_order_amount NUMBER(12, 0) DEFAULT 0 NOT NULL,
...
CREATE TABLE user_coupon (
  ...
  CONSTRAINT ck_user_coupon_used CHECK (used IN (0, 1))
);
...
CREATE TABLE payment_order (
  ...
  points_used NUMBER(12, 0) DEFAULT 0 NOT NULL,
  points_earned NUMBER(12, 0) DEFAULT 0 NOT NULL,
  coupon_discount NUMBER(12, 0) DEFAULT 0 NOT NULL,
  ...
  CONSTRAINT fk_payment_buyer FOREIGN KEY (buyer_id) REFERENCES app_user (id),
  CONSTRAINT fk_payment_product FOREIGN KEY (procedure_product_id) REFERENCES procedure_product (id),
  CONSTRAINT fk_payment_user_coupon FOREIGN KEY (user_coupon_id) REFERENCES user_coupon (id),
  CONSTRAINT ck_payment_status CHECK (status IN ('READY', 'PAID', 'CANCELED'))
);
...
CREATE TABLE point_history (
  ...
  amount NUMBER(12, 0) NOT NULL,
  balance_after NUMBER(12, 0) NOT NULL,
  order_id NUMBER(19),
  memo VARCHAR2(200 CHAR),
  created_at TIMESTAMP DEFAULT SYSTIMESTAMP NOT NULL,
  CONSTRAINT pk_point_history PRIMARY KEY (id),
  CONSTRAINT fk_point_history_user FOREIGN KEY (user_id) REFERENCES app_user (id),
  CONSTRAINT fk_point_history_order FOREIGN KEY (order_id) REFERENCES payment_order (id),
  CONSTRAINT ck_point_history_type CHECK (type IN ('EARN', 'USE'))
);

CREATE INDEX ix_review_created_at ON review (created_at DESC);
CREATE INDEX ix_notice_created_at ON notice (created_at DESC);
CREATE INDEX ix_qna_created_at ON qna_post (created_at DESC);
CREATE INDEX ix_payment_created_at ON payment_order (created_at DESC);
CREATE INDEX ix_consult_created_at ON quick_consultation (created_at DESC);
CREATE INDEX ix_point_history_created_at ON point_history (created_at DESC);
