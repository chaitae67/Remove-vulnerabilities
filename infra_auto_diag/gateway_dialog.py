...
def open_gateway(app):
    ...
    ttk.Label(dlg, text="private 서브넷 서버는 bastion을 점프호스트로 경유.")\
        .grid(row=0, column=0, columnspan=4, sticky="w", padx=6)

    ttk.Label(dlg, text="Bastion IP").grid(row=1, column=0, sticky="e", **pad)
    e_host = ttk.Entry(dlg, width=24); e_host.insert(0, g["host"]); e_host.grid(row=1, column=1, **pad)
    ttk.Label(dlg, text="포트").grid(row=1, column=2, sticky="e", **pad)
    e_port = ttk.Entry(dlg, width=6); e_port.insert(0, str(g["port"])); e_port.grid(row=1, column=3, sticky="w", **pad)

    ttk.Label(dlg, text="계정(ID)").grid(row=2, column=0, sticky="e", **pad)
    e_user = ttk.Entry(dlg, width=24); e_user.insert(0, g["user"]); e_user.grid(row=2, column=1, **pad)

    auth = tk.StringVar(value=g["auth"])
    af = ttk.Frame(dlg); af.grid(row=3, column=0, columnspan=4, sticky="w", padx=6)
    ...
    ttk.Label(dlg, text="비밀번호").grid(row=4, column=0, sticky="e", **pad)
    e_pass = ttk.Entry(dlg, width=24, show="*"); e_pass.insert(0, g["password"]); e_pass.grid(row=4, column=1, **pad)

    ttk.Label(dlg, text="키 파일").grid(row=5, column=0, sticky="e", **pad)
    e_key = ttk.Entry(dlg, width=30); e_key.insert(0, g["key"]); e_key.grid(row=5, column=1, columnspan=2, sticky="we", **pad)
    ...
    ttk.Button(dlg, text="찾기", width=6, command=browse).grid(row=5, column=3, **pad)
